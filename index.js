...
function sortByNamesAndCapitalizeFirstLetter(arr) {
  ...
  let arrayOnlyValues = arraySortedByName.map((element) =>
...
function transformToSnakeCaseAndAllCaps(arr) {
  ...
  let snakeString = lodash.snakeCase([arr]); //hello_world
  let finalString = lodash.toUpper(snakeString); // HELLO_WORLD
  return finalString.split(" "); //[ 'HELLO_WORLD' ]
}
